...
/* select sql .
    ...
    ㄴ 'ResultSet' 타입 객체는 행 집합을 저장합니다. select 쿼리에서 사용되는 리턴 타입.
      - ResultSet 객체는 현재 데이터 행을 가리키는 '커서'를 유지합니다.
...
public class ResultSetTest {
  ...
  public static void selectByLevel() { // 같은 레벨(step) 값이 여러 개 있으므로 조회 되는 행은 0 ~ 여러개
    ...
    String step = System.console().readLine("조회 레벨 입력 >>> "); // "public";
    ...
    try (
      ...
      ResultSet rs = pstat.executeQuery(); // 🔥 SELECT 실행 메소드 executeQuery 실행결과(행 집합)는 ResultSet 타입
      boolean result = rs.next(); // 처음 rs 커서는 첫번쨰 앞에 있으므로 다음 이동 필요.
      int count = 0;
  ...
  public static void selectByPk() { // 조회 결과 0 또는 1개 행
    System.out.println("-------- tbl_javadict 테이블 조회하기 ---------");
    String english = System.console().readLine("조회 영단 입력 >>> "); // "public";
    ...
    try (
      ...
      if (result) {
        System.out.println("#### 조회 결과 컬럼값 출력 ####");
        // rs.getXXXX(컬럼인덱스) : XXXX 은 컬럼이 number 타입이면 Int,Double char 또는 varchar2 타입이면
        // String
        System.out.println("번호 : " + rs.getInt(1));
        System.out.println("영어 단어 : " + rs.getString(2));
        System.out.println("한글 뜻 : " + rs.getString(3));
