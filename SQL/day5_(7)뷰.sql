 ...
 * 		- 가상 테이블
 * 		- 자주 사용하는 select 조회 결과를 미리 컴파일하여 뷰 객체로 저장
...
JOIN 
  ...
  ) sale        -- group by 결과 행 집합의 별칭(테이블처럼 서브쿼리로 사용)
ON 	tp.pcode = sale.pcode
ORDER BY total DESC;

-- 위의 쿼리를 자주 사용한다면 미리 컴파일하여 veiw 객체로 만듭니다.
-- 그러면 좀 더 쿼리가 빠르고 단순해 집니다
-- view 생성은 create or replace 명령을 사용하여 수정이 필요할 떄 drop 없이 다시 정의
