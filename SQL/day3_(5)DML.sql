-- DML: selecr, insert, update, delete
--      테이블의 행을 대상으로 하는 명령어
...
--          ㄴ where이 없으면 모든 값을 update 하거나 delete 해버리기 때문

-- insert, update, delete 는 변경사항에 대해 바로 테이블에 반영(커밋) 하지않고
--          트랜잭션 관리 차원에서 commit 명령어로 수행하는 바람직!!! => 실무
--          rollback 은 마지막 commit 후에 실행한 DML 을 취소(되돌리기) 합니다.
--          트랜잭션: 하나의 논리적인 업무를 수행하는 단위(여러개의 DML 명령으로 구성)
--          autocommit 설정이 false 이면 다른 세션(클라이언트-sqlplus,dbeaver, ...) commit 하기 전까지는
--                  변경된 데이터를 확인할 수 없습니다.(반영 X)

-- 트랜잭션(transaction): 데이터베이스에서 여러 작업을 하나의 논리적인 작업 단위로 묶는 것
--        ㄴ 즉, 모두 성공(commit)하거나 모두 실패(rollback)하는 작업 단위를 의미
...
SELECT * FROM TBL_STUDENT WHERE STUNO='2025003';        -- 변경내용 확인
