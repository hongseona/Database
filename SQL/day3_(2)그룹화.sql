...
/*
    그룹화: 지정된 컬럼값이 같은 행들을 하나의 그룹으로 묶고
            집계 함수를 그룹 단위로 적용.
    지정된 칼럼은 그룹화 컬럼: 대체로 값이(중복) 많은 컬럼들이 대상이 됩니다.
            그룹화 컬럼에 대해 집계 함수를 적용

            SELECT 그룹화컬럼, 함수(Number컬럼)
            FROM 테이블명
            [WHERE] 조건식(테이블의 모든 행/컬럼 대상)        ** 조건 필요에 따라 사용(필요없다면 사용 X)
            [ORDER BY] 정렬 컬럼(그룹화 하기전에 정렬 - 의미없음)
            GROUP BY 그룹화 컬럼 🔥
            [HAVING] 조건식(select 조회 컬럼만 대상으로 합니다.)
...
-- CAR 컬럼으로 그룹화: 조회는 함수를 적용한 결과 입니다
SELECT car, COUNT(*)
FROM TBL_CAR_CO2
GROUP BY car;

-- avg 함수 실행이 가능한 모든 컬럼 사용
SELECT car, round(avg(volume)),round(avg(WEIGHT)),round(avg(CO2))       -- round: 정수값으로 변환
FROM TBL_CAR_CO2
GROUP BY car;
...
FROM TBL_CAR_CO2
GROUP BY car
HAVING round(avg(CO2)) < 100        -- HAVING 조건식에는 별칠 사용 못합니다
-- ORDER BY round(avg(CO2));       -- HAVING 으로 값 추출한 후에 정렬(ORDER BY)
ORDER BY "CO2평균";
