...
마치 책의 목차/인덱스처럼, 원하는 데이터를 빠르게 찾을 수 있게 해줌
...
SELECT *
FROM tbl_javadict
WHERE idx = 2;      -- 기본키 컬럼 검색(인덱스를 통해서 빠른 성능)
...
-- 새로운 연습. 기본키가 없는 테이블에 기본키를 설정하기
-- PK 의 자격: NULL 값이 없음. UNIQUE(고유성)
--              ㄴ model 컬럼이 있으나 충분하지 않다. (미래의 값이 고유성 예측이 안됨)
--              ㄴ 해결책1: 새로운 컬럼 idx를 만든다. -> PK
--              ㄴ 해결책2: 기존 컬럼만으로 한다면 (car,model) 복합 컬럼으로 PK를 만든다.

ALTER TABLE TBL_CARCO2 ADD CONSTRAINT pk_carco2 PRIMARY KEY (car,model);

--  (car, model) 그룹으로 묶었을 때 같은 값이 있으면 기본키를 설정할 수 없다.
