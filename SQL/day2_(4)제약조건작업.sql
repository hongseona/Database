...
--> USER_CONSTRAINTS, USER_CONS_COLUMNS 테이블은 오라클이 만든 시스템 테이블  => '딕셔너리 뷰' 라고 부름

*/

-- USER_CONSTRANINTS 테이블: 사용자가 만든 테이블의 제약 조건 정보를 저장
SELECT *
FROM USER_CONSTRAINTS
WHERE TABLE_NAME = 'TBL_JAVADICT';
-- TABLE_NAME = 'TBL_JAVAWORD'; 는 해당 테이블의 제약 조건이 없으므로 조회 결과가 없습니다.          
...
/*
 ✅2. 제약 조건 적용을 변경하기
 : alter table 명령어로 합니다. (alter: 바꾸다, 개조하다)
 : alter table 는 이미 있는 테이블의 구조를 변경할 떄 사용합니다.(컬럼 추가/삭제/변경, 제약조건 변경, 이름변경 등)
 : ㄴ 즉, CREATE TABLE 로 만든 테이블의 구조를 수정할때 사용
*/
...
-- 제약조건 unique, pk(primary key), check 는 복사가 안됩니다.
-- 제약조건 추가하기: ALTER TABLE 테이블명 ADD CONSTRAINT 제약조건 이름 <제약조건 내용>
--                  제약조건은 우리(개발자)가 임의로 정해줍니다. ALTER
