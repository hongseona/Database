...
CREATE TABLE tbl_javaword (
            idx number(10),
            english varchar2(100),  --varchar2 는 가변길이. 최대 100바이트
...
-- 2) DML
-- insert: 행(row) 추가
-- insert 형식1: 컬럼명 안쓸때는 '모든값을 컬럼 정의 순서대로' 나열
...
-- insert 형식2: 선택적으로 컬럼 이름을 지정할 수 있습니다
...
-- 일부 행 추출: 1) 전체 행(조건이 없을 떄)     2) 조건에 맞는 행
...
-- 예시2: idx 값이 null 인 행을 조회
SELECT * FROM TBL_JAVAWORD
                WHERE idX IS null;

-- 예시3: english 값이 'private' 인 행을 조회
SELECT * FROM TBL_JAVAWORD
                WHERE ENGLISH = 'private';

-- 예시4: korean 값에 '반대' 포함 (like와 %) 한 행을 조회
SELECT * FROM TBL_JAVAWORD
...
-- 2개 이상의 컬러으로 조건식 만들기 가능합니다. : and, or
SELECT * FROM TBL_JAVAWORD
