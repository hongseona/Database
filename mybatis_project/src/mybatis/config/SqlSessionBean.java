...
/*
 * ✅ SqlSessionFactory
 * MyBatis의 핵심 객체로, 데이터베이스 연결을 수행. SQL 실행을 담당하는 SqlSession을 생성합니다.
 * Connection 역할
 * ✅ SqlSession
 * 실제 SQL 실행을 담당하는 객체입니다. 데이터베이스와의 연결을 관리하고 트랜잭션을 처리합니다.
 * PreparedStatement 역할
 * 🔥 SQL Mapper 를 설정하는 방식: 1)문자열로 지정하기 vs 2)매퍼인터페이스 만들기
 * 1)의 특징: 오류에 대한 검증을 못함
 * 2)의 특징: 잘못 지정하면 컴파일 오류 발생. 협업에서 필수 - spring 에서는 인터페이스 필수 사용
 * ✅ Mapper Interface
